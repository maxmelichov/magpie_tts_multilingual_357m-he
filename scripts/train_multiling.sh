#!/usr/bin/env bash
# Run 5: LoRA fine-tune on English/German/Italian/Spanish with IPA input.
#
# These four languages are ALREADY supported by the base model through its own
# g2p pipeline, which is the point: it lets us compare the base model's WER
# against an IPA LoRA on identical audio, instead of only being able to claim it
# for a language the base model never had (Hebrew).
#
...
DATASETS_JSON="$REPO_ROOT/data/manifests_multiling/datasets.json"
...
mapfile -t DS_ARGS < <("$REPO_ROOT/venv/bin/python" "$REPO_ROOT/scripts/ds_meta_args.py" "$DATASETS_JSON" per-language)
...
exec "$REPO_ROOT/venv/bin/python" "$REPO_ROOT/scripts/magpietts_lora.py" \
    ...
    +extra_tokenizers_json="$REPO_ROOT/data/manifests_multiling/tokenizers.json" \
    +lora.r="$LORA_R" \
